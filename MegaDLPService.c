...
//           udev_device_get_devnode(dev));
    ...
    char now[64];
    size_t ret = strftime(now, sizeof(now), "%c", tm);
    assert(ret);

    FILE *f = fopen("./Log/logUSBDevice.txt", "a");
    fprintf(f, "%s %s %s %6s %s:%s %s \n", now, udev_device_get_subsystem(dev),udev_device_get_devtype(dev),action,vendor, product, udev_device_get_devnode(dev));
    fclose(f);
